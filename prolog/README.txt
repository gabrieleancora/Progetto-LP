...
nfa_regexp_comp/2
    nfa_regexp_comp(FA_ID, RE) => Dall'input RE crea il suo automa corrispondente, e lo denomina FA_ID.
            Ogni nodo dell'automa è generato automaticamente, e gli archi avranno un tag nullo (epsilon) o il tag relativo all'espressione,
            in base alla costruzione necessaria dell'operatore.
        
            nfa_regexp_comp utilizza una funzione di supporto chiamata crea_stati_segno,
            che in base all'operatore da utilizzare creerà ricorsivamente il pezzo di automa relativo all'operatore stesso.         
